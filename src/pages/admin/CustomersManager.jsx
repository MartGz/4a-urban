...
const CustomersManager = () => {
  ...
  return (
    <div className="space-y-12 pb-20">
      <div>
        <h1 className="font-urban text-3xl sm:text-5xl uppercase tracking-tighter font-bold">Base de Datos</h1>
      ...
      <div className="bg-white/[0.02] border border-white/5 rounded-[24px] sm:rounded-[40px] overflow-hidden">
        <div className="p-5 sm:p-10 border-b border-white/5 flex justify-between items-center bg-white/[0.01]">
          <div className="flex items-center gap-4">
            <div className="p-3 bg-white/5 rounded-2xl text-white/40">
              <Users size={20} />
            </div>
            <span className="font-urban text-lg sm:text-2xl uppercase tracking-tighter font-bold">{customers.length} Clientes Activos</span>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-white/[0.01] border-b border-white/5 text-white/20 text-[9px] uppercase tracking-[0.3em] font-urban whitespace-nowrap">
                <th className="px-4 sm:px-6 lg:px-10 py-4 sm:py-6 font-normal">Identidad</th>
                <th className="px-4 sm:px-6 lg:px-10 py-4 sm:py-6 font-normal">Contacto</th>
                <th className="px-4 sm:px-6 lg:px-10 py-4 sm:py-6 font-normal">Cumpleaños</th>
                <th className="px-4 sm:px-6 lg:px-10 py-4 sm:py-6 font-normal">Inversión</th>
                <th className="px-4 sm:px-6 lg:px-10 py-4 sm:py-6 font-normal text-right">Detalles</th>
              </tr>
            </thead>
              ...
              {customers.map((user) => (
                <Fragment key={user.id}>
                  <tr className={`border-b border-white/5 hover:bg-white/[0.02] transition-colors group cursor-pointer ${expandedId === user.id ? 'bg-white/[0.03]' : ''}`}
                    ...
                    <td className="px-4 sm:px-6 lg:px-10 py-5 sm:py-6 lg:py-8 whitespace-nowrap">
                      <div className="flex items-center gap-5">
                        <div className="w-12 h-12 shrink-0 bg-white/5 rounded-2xl flex items-center justify-center text-white/20 font-bold text-xl group-hover:bg-white group-hover:text-black transition-all">
                  ...
                  <AnimatePresence>
                    {expandedId === user.id && (
                      <tr>
                        <td colSpan="5" className="px-4 sm:px-6 lg:px-10 py-0 bg-black/40">
                          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                             <div className="py-6 sm:py-10 border-x border-white/5">
                                <div className="flex items-center gap-4 mb-8 px-4 sm:px-10">
                                   <Package size={16} className="text-white/20 shrink-0" />
                                   <h4 className="text-[10px] uppercase tracking-[0.4em] text-white/40 font-bold whitespace-nowrap">Historial de pedidos del cliente</h4>
      ...
      <div className="grid md:grid-cols-2 gap-6 sm:gap-8">
        <div className="bg-gradient-to-br from-amber-500/10 to-transparent border border-amber-500/20 p-6 sm:p-10 rounded-[24px] sm:rounded-[40px] relative overflow-hidden group">
        ...
        <div className="bg-gradient-to-br from-emerald-500/10 to-transparent border border-emerald-500/20 p-6 sm:p-10 rounded-[24px] sm:rounded-[40px] relative overflow-hidden group">
